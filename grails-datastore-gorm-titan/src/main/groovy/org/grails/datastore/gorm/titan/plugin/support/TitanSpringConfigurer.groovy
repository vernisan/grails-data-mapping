...
import org.grails.datastore.gorm.titan.bean.factory.TitanMappingContextFactoryBean
import org.grails.datastore.gorm.titan.bean.factory.TitanDatastoreFactoryBean
 ...
 * @since 1.0
 */
class TitanSpringConfigurer extends SpringConfigurer {

    static neo4jDefaultLocation = "data/neo4j"
    public static final String JDBC_NEO4J_PREFIX = "jdbc:titan:instance:"
    ...
    Closure getSpringCustomizer() {
        return {
            ...
            def config = application.config?.grails?.titan ?: new ConfigObject()

            def neo4jUrl = config?.url ?: "jdbc:titan:mem"
            def neo4jProperties = [:]
            ...
            neo4jMappingContext(TitanMappingContextFactoryBean) {
                grailsApplication = ref('grailsApplication')
            }

            neo4jDatastore(TitanDatastoreFactoryBean) {
