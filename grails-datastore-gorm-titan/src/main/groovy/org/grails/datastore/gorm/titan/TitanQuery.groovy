...
import org.grails.datastore.gorm.titan.CypherBuilder;
import org.grails.datastore.gorm.titan.TitanEntityPersister;
...
@CompileStatic
@Slf4j
class TitanQuery extends Query {

    final TitanEntityPersister titanEntityPersister

    protected TitanQuery(Session session, PersistentEntity entity, TitanEntityPersister titanEntityPersister) {
        super(session, entity)
        this.titanEntityPersister = titanEntityPersister
    }
    ...
    protected List executeQuery(PersistentEntity persistentEntity, Junction criteria) {
        ...
        if (projections.projectionList.empty) {
            return executionResult.collect { Map<String,Object> map ->   // TODO: potential performance problem here: for each instance we unmarshall seperately, better: use one combined statement to get 'em all
                ...
                titanEntityPersister.unmarshallOrFromCache(persistentEntity, id, labels, data)
            }
        } else {
    ...
    def buildConditionsPropertyCriterion( PropertyCriterion pnc, CypherBuilder builder, String prefix) {
        int paramNumber = builder.addParam(TitanUtils.mapToAllowedNeo4jType(pnc.value, entity.mappingContext))
        def rhs
        def lhs
        def operator
        ...
        switch (pnc) {
            ...
            case Between:
                ...
                int paramNumberFrom = builder.addParam(TitanUtils.mapToAllowedNeo4jType(b.from, entity.mappingContext))
                int parmaNumberTo = builder.addParam(TitanUtils.mapToAllowedNeo4jType(b.to, entity.mappingContext))
