 ...
 */
class TitanGormEnhancer extends GormEnhancer {

    public static final String UNDECLARED_PROPERTIES = "_neo4j_gorm_undecl_"

    TitanGormEnhancer(Datastore datastore, PlatformTransactionManager transactionManager = null) {
        super(datastore, transactionManager)
    }

    @Override
    TitanDatastore getDatastore() {
        return super.getDatastore() as TitanDatastore
    }

    protected <D> GormStaticApi<D> getStaticApi(Class<D> cls) {
        return new TitanGormStaticApi<D>(cls, datastore, finders, transactionManager)
    }

    protected <D> GormInstanceApi<D> getInstanceApi(Class<D> cls) {
        final api = new TitanGormInstanceApi<D>(cls, datastore)
    ...
    public static Map<String, List<Object>> amendMapWithUndeclaredProperties(Map<String, Object> simpleProps, Object pojo, MappingContext mappingContext) {
        ...
        Map<String,Object> map = (Map) obj.getProperty(TitanGormEnhancer.UNDECLARED_PROPERTIES);
        if (map!=null) {
            for (Map.Entry<String,Object> entry : map.entrySet()) {
                if  (mappingContext.isPersistentEntity(entry.getValue())) {
                    ...
                    simpleProps.put(entry.getKey(), TitanUtils.mapToAllowedNeo4jType(entry.getValue(), mappingContext));
                }
            }
        }
        dynRelProps
...
class TitanGormInstanceApi<D> extends GormInstanceApi<D> {

    TitanGormInstanceApi(Class<D> persistentClass, Datastore datastore) {
    ...
    def propertyMissing(D instance, String name) {
        ...
        MetaProperty mp = unwrappedInstance.hasProperty(TitanGormEnhancer.UNDECLARED_PROPERTIES);
    ...
    def propertyMissing(D instance, String name, val) {
        ...
        if (name == TitanGormEnhancer.UNDECLARED_PROPERTIES) {
            unwrappedInstance.metaClass."${TitanGormEnhancer.UNDECLARED_PROPERTIES}" = val
        } else {
            ...
            if (mp) {
                ...
                undeclaredProps = [:]
                unwrappedInstance.metaClass."${TitanGormEnhancer.UNDECLARED_PROPERTIES}" = undeclaredProps
            }
            (val == null) ? undeclaredProps.remove(name) : undeclaredProps.put(name, val)
            if (datastore.mappingContext.isPersistentEntity(val)) {
                val.save()
            } else if (TitanGormEnhancer.isCollectionWithPersistentEntities(val, datastore.mappingContext)) {
                val.each { it.save() }
            }
    ...
    def cypher(instance, String queryString, Map params ) {
        ...
        ((TitanDatastore)datastore).cypherEngine.execute(queryString, params)
    }
    ...
    def cypher(instance, String queryString) {
        ((TitanDatastore)datastore).cypherEngine.execute(queryString, Collections.singletonList(instance.id))
    }

}

class TitanGormStaticApi<D> extends GormStaticApi<D> {

    TitanGormStaticApi(Class<D> persistentClass, TitanDatastore datastore, List<FinderMethod> finders, PlatformTransactionManager transactionManager) {
    ...
    def cypherStatic(String queryString) {
        ((TitanDatastore)datastore).cypherEngine.execute(queryString)
    }
