...
import org.grails.datastore.gorm.titan.TitanQuery;
import org.grails.datastore.gorm.titan.engine.CypherEngine;
import org.grails.datastore.gorm.titan.engine.CypherResult;
import org.grails.datastore.gorm.titan.parsers.PlingStemmer;
 ...
 */
public class TitanEntityPersister extends EntityPersister {

    private static Logger log = LoggerFactory.getLogger(TitanEntityPersister.class);


    public TitanEntityPersister(MappingContext mappingContext, PersistentEntity entity, Session session, ApplicationEventPublisher publisher) {
        super(mappingContext, entity, session, publisher);
    }

    @Override
    public TitanSession getSession() {
        return (TitanSession) super.session;
    }
    ...
    protected List<Object> retrieveAllEntities(PersistentEntity pe, Iterable<Serializable> keys) {
        ...
        return new TitanQuery(session, pe, this).executeQuery(pe, junction);
    }
    ...
    protected Object retrieveEntity(PersistentEntity pe, Serializable key) {
        ...
        return IteratorUtil.singleOrNull(new TitanQuery(session, pe, this).executeQuery(pe, new Conjunction(criteria)).iterator());
    }
...
//                        }
                ...
                if (entry.getKey().isOutgoing()) {
                    ...
                    while (idIter.hasNext() && labelIter.hasNext()) {
                        ...
                        Object proxy = getMappingContext().getProxyFactory().createProxy(
                                session,
                                ((TitanMappingContext) getMappingContext()).findPersistentEntityForLabels(labels).getJavaClass(),
                                targetId
                        );
                        values.add(proxy);
                    }
        ...
        if (!data.isEmpty()) {
            ...
            go.setProperty(TitanGormEnhancer.UNDECLARED_PROPERTIES, data);
        }
...
//            return null;
    ...
    public Query createQuery() {
        return new TitanQuery(session, getPersistentEntity(), this);
    }
