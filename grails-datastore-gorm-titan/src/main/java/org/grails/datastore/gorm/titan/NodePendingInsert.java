...
import org.grails.datastore.gorm.titan.TitanUtils;
    ...
    public void run() {
        ...
        for (PersistentProperty pp : getEntityAccess().getPersistentEntity().getPersistentProperties()) {
            if (pp instanceof Simple) {
                ...
                if (value != null) {
                    simpleProps.put(name, TitanUtils.mapToAllowedNeo4jType( value, mappingContext));
                }
            }
        }

        Map<String, List<Object>> dynamicRelProps = TitanGormEnhancer.amendMapWithUndeclaredProperties(simpleProps, getNativeEntry(), mappingContext);
